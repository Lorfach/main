...
export const Video = memo(({ YT_url, RU_url }) => {
    ...
    const buttonStyles = (isActive) => 
        `cursor-pointer select-none py-3 px-6 border border-[#63697a] text-white rounded-xl ${isActive ? 'bg-[#c5c5c5] text-[#1e1e1f]' : ''}`;

    const extractYouTubeId = (url) => {
        const match = url?.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|live\/|v\/|u\/\w\/))([a-zA-Z0-9_-]{11})/);
    ...
    return (
        <>
            <div className="ml-4 md:mx-auto md:w-9/12 flex gap-4 font-[GolosText-Regular]">
                {YT_url && (
                    <div onClick={() => setVideo(0)} className={buttonStyles(!video)}>YouTube</div>
                )}
                {RU_url && (
                    <div onClick={() => setVideo(1)} className={buttonStyles(video)}>RUTUBE</div>
                )}
            </div>
